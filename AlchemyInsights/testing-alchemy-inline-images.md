---
title: Testen Alchemy inline images
ms.author: pebaum
author: pebaum
ms.date: 11/16/2017
ms.audience: ITPro
ms.topic: article
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.assetid: 28d8036f-9df2-4dff-835b-9424bfe988a2
ms.custom:
- "273"
- "1500006"
ms.openlocfilehash: 8a9d98e2af3ef0cc71b820b4e74aa2a2a522346b
ms.sourcegitcommit: 0b06093dabd685f76cc39b1d7c0f8b03883b6e79
ms.translationtype: MT
ms.contentlocale: nl-NL
ms.lasthandoff: 10/25/2019
ms.locfileid: "36743124"
---
# <a name="access-the-domains-page"></a><span data-ttu-id="ddfeb-102">De pagina domeinen openen</span><span class="sxs-lookup"><span data-stu-id="ddfeb-102">Access the Domains Page</span></span>

<span data-ttu-id="ddfeb-103">Als u de pagina domeinen wilt openen, klikt u op dit onderstaande pictogram:</span><span class="sxs-lookup"><span data-stu-id="ddfeb-103">To access the domains page, just click this icon below:</span></span>

  
<img src='data:image/png;base64,'/>

<span data-ttu-id="ddfeb-104">Vervolgens u uw domein toevoegen, wijzigen of verwijderen.</span><span class="sxs-lookup"><span data-stu-id="ddfeb-104">Then you can add, modify, or remove your domain.</span></span>
<span data-ttu-id="ddfeb-105">Zie de [Veelgestelde vragen over domeinen](https://docs.microsoft.com/office365/admin/setup/domains-faq) voor meer informatie.</span><span class="sxs-lookup"><span data-stu-id="ddfeb-105">For more information, see the [Domains FAQ.](https://docs.microsoft.com/office365/admin/setup/domains-faq)</span></span>
  